function SetupPsychtoolboxAuto
...
while any(regexp(path, searchpattern))
    ...
    answer='yes';
    % input('Before you decide to delete the paths, do you want to see them (yes or no)? ','s');
    if ~strcmpi(answer,'yes') && ~strcmpi(answer,'y')
    ...
    answer='yes';
    %input('Shall I delete all those instances from the MATLAB/OCTAVE path (yes or no)? ','s');
    if ~strcmpi(answer,'yes') && ~strcmpi(answer,'y')
